...
import { 
    AuthResponse, User, Post, Comment, 
    UsersResponse, PostsResponse, CommentsResponse,
    AnalyticsCache, AuthCredentials
} from '../types';
...
export class AnalyticsService {
    ...
    private async authenticate(): Promise<void> {
        try {
            ...
            const response = await this.axiosInstance.post<AuthResponse>('/auth', this.authCredentials);
    ...
    public async updateCache(): Promise<void> {
        try {
            ...
            const usersResponse = await this.makeRequest<UsersResponse>('get', '/users');
            ...
            const postsPromises = users.map(user => 
                this.makeRequest<PostsResponse>('get', `/users/${user.id}/posts`)
            );
            ...
            const commentsPromises = allPosts.map(post =>
                this.makeRequest<CommentsResponse>('get', `/posts/${post.id}/comments`)
            );
    ...
    public getTopPosts(): Post[] {
        if (this.cache.postCommentCounts.size === 0) return [];
